...
node {
    ...
    stage('packaging back-office') {
        sh "./mvnw -ntp verify -P-webpack -Pprod -DskipTests"
        archiveArtifacts artifacts: '**/target/back/*.war', fingerprint: true
    }
    stage('packaging front-office') {
        sh "./mvnw -ntp verify -P-webpack -Pprod-front -DskipTests"
        archiveArtifacts artifacts: '**/target/front/*.war', fingerprint: true
    }
