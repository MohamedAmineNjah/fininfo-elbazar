...
export const ACTION_TYPES = {
  ...
  FETCH_CLIENT_ADRESSES_LIST: 'client/FETCH_CLIENT_ADRESSES_LIST',
  FETCH_FILTRED_CLIENT_LIST: 'client/FETCH_FILTRED_CLIENT_LIST',
...
const initialState = {
  ...
  clientAdresses: [] as ReadonlyArray<IAdresse>,
  entity: defaultValue,
  updating: false,
  totalItems: 0,
  updateSuccess: false,
...
export default (state: ClientState = initialState, action): ClientState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.FETCH_CLIENT_ADRESSES_LIST):
    case REQUEST(ACTION_TYPES.FETCH_FILTRED_CLIENT_LIST):
    ...
    case FAILURE(ACTION_TYPES.FETCH_CLIENT_ADRESSES_LIST):
    case FAILURE(ACTION_TYPES.FETCH_FILTRED_CLIENT_LIST):
    ...
    case SUCCESS(ACTION_TYPES.FETCH_FILTRED_CLIENT_LIST):
      return {
        ...state,
        loading: false,
    ...
    case SUCCESS(ACTION_TYPES.FETCH_CLIENT_ADRESSES_LIST):
      return {
        ...state,
        loading: false,
        clientAdresses: action.payload.data,
        totalItems: parseInt(action.payload.headers['x-total-count'], 10),
      };
...
const apiAdressesUrl = 'api/adresses/client';
...
export const getFilteredEntities: ICrudGetAllFiltredAction<IClient> = (filter, page, size, sort) => {
  return {
    type: ACTION_TYPES.FETCH_FILTRED_CLIENT_LIST,
    payload: axios.get<IClient>(`${apiUrl}?${filter}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`),
  };
};
...
export const getEntityAdresses: ICrudGetAction<IAdresse> = id => {
  const requestUrl = `${apiAdressesUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_CLIENT_ADRESSES_LIST,
